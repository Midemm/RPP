...
conn = psycopg2.connect(dbname="knowledge_base_for_kolesnikov", user="kolesnikov_knowledge_base",
                        password="9999", host="127.0.0.1")
cursor = conn.cursor()
...
def is_admin(chat_id):
    cursor.execute("SELECT chat_id FROM admins WHERE chat_id = %s::VARCHAR", (str(chat_id),))
...
# Код первого микросервиса для управления валютами

async def load_currency(request):
    ...
    cursor.execute(f"SELECT * FROM currencies WHERE currency_name = '{currency_name}'")
    if cursor.fetchone():
        return web.Response(status=400, text=f"Currency {currency_name} already exists.")
...
async def update_currency(request):
    ...
    if not cursor.fetchone():
        return web.Response(status=400, text=f"Currency {currency_name} does not exist.")

    # Обновление курса валюты
    cursor.execute(f"UPDATE currencies SET rate = {new_rate} WHERE currency_name = '{currency_name}';")
    conn.commit()
...
async def start_microservice():
    app = web.Application()
    app.add_routes([web.post('/load_currency', load_currency),
                    web.post('/update_currency', update_currency),
                    web.post('/delete_currency', delete_currency)])
    runner = web.AppRunner(app)
    await runner.setup()
    site = web.TCPSite(runner, 'localhost', 5001)
    await site.start()

# Код второго микросервиса для конвертации валюты

async def convert_currency(request):
    currency_name = request.query.get('currency_name')
    ...
    return web.json_response({'converted_amount': result})
...
async def get_currencies(request):
    with conn.cursor() as cursor:
        ...
        if not currencies:
            return web.Response(status=404, text="No currencies found.")

        currency_list = [currency[0] for currency in currencies]
        return web.json_response({'currencies': currency_list})


async def start_microservice():
    app = web.Application()
    app.add_routes([web.get('/convert', convert_currency),
                    web.get('/currencies', get_currencies)])
    runner = web.AppRunner(app)
    await runner.setup()
...
if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    asyncio.run(start_microservices())
